...
consumed and understood. They showcase the current status of the enterprise network along with 
all the associated SLAs that are considered critical to the business.
Let’s suppose there were some network issues reported yesterday in one if the corporate internal 
production systems. As the Network Manager was starting his day, he wanted to access the tool 
himself to make sure that the issue was resolved and there are no reoccurrences of the problem 
since it was addressed and resolved. 
Also, the Network Manager wants to validate that the network is healthy and review if there are 
any SLAs being breached in any of their remote corporate offices.
